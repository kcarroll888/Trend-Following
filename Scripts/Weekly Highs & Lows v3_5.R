...
positions <- c("ADE.OL", "ADS.DE", "AMZN", "AO.L", "ATO.PA", "DPW.DE",
               "ECL", "HSBA.L", "IGG.L", "JUST.L", "LIGHT.AS", "MDT",
               "NDAQ", "NIO", "NKE", "OKTA", "PETS.L", "ROST",
               "SAN.MC", "SCT.L", "TPR", "VFC", "VWS.CO")
...
myVol <- function(ticker, period = 40) {
  ...
  vol <- volatility(s[, c("Open", "High", "Low", "Close")],
                    n = period, calc = "yang.zhang")
  names(vol) <- "Vol"
  vol$mean <- mean(vol, na.rm = TRUE)
  return(vol)
}

mySNR <- function(Close, lookBack) {
  # Test for case when number of weeks stock has been trading is less
  #  than lookBack period
  if (length(Close) > lookBack) {
    # to use the dplyr lag function need to work on data frame, not a xts object
    Close <- data.frame(Close = Close, stringsAsFactors = FALSE)
    Close <- mutate(Close, absPxChg = abs(Close - lag(Close, 1)))
    l <- nrow(Close) # most recent observation is last object
    r <- abs(Close$Close[l] - Close$Close[l - lookBack]) /
         sum(Close$absPxChg[l:(l - lookBack)])
    } else {
            r <- NA
           }
  return(r)
...
cleanUp <- function(series, magnitude = 10) {
  ...
  #  1) Sometimes Yahoo data contains an obvious error e.g. 20130 instead
  #     of 201.3. To remedy, compare each row of Open with the mean of the
  #     High, Low & Close. Is it an order of magnitude out? e.g. greater
  #     than 10x? If so use the highest of the other values as a substite.
  #     repeat with the High, Low and Close data.

  # TO DO: Need to test which column has the most skewed data and clean
  #        that first
  magHigh <- (series$High / mean(series$Open + series$Low + series$Close)
              > magnitude) |
             (series$High / mean(series$Open + series$Low + series$Close)) < (1 / magnitude)
  series$High[magHigh] <- max(series[magHigh, c("Open", "Low", "Close")])

  magOpen <- (series$Open / mean(series$High + series$Low + series$Close)
              > magnitude) |
             (series$Open / mean(series$High + series$Low + series$Close)) < (1 / magnitude)
  series$Open[magOpen] <- mean(series[magOpen, c("High", "Low", "Close")])

  magLow <- (series$Low / mean(series$Open + series$High + series$Close)
             > magnitude) |
            (series$Low / mean(series$Open + series$High + series$Close)) < (1 / magnitude)
  series$Low[magLow] <- min(series[magLow, c("Open", "High", "Close")])

  magClose <- (series$Close / mean(series$Open + series$High + series$Low)
               > magnitude) |
                (series$Close / mean(series$Open + series$High + series$Low)) < (1 / magnitude)
  series$Close[magClose] <- mean(series[magClose, c("Open", "High", "Low")])
...
chartBreakout <- function(symbol, period = 40, timeF = "W", ed = Sys.Date()) {
  ...
  # Add 1 to the date as call to getSymbols returns data date - 1
  endDate <- getLastFri(Sys.Date()) + 1
  ...
  if (timeF == "W") {
    ...
    symbolData <- getYH_API(symbol)

    # Convert to weekly format
    names(symbolData) <- c("Open", "High", "Low", "Raw Close",
                           "Volume", "Close")
    ...
    names(symbolData) <- c("Open", "High", "Low", "Raw Close",
                           "Volume", "Close")
    symbolData <- symbolData[, c("Open", "High", "Low", "Close",
                                "Volume")]

    } else {
          stop("Timeframe must be 'W' for weekly or 'D' for daily")
           }

  # Cleanup the data
  symbolData[, c("Open", "High", "Low", "Close")] <-
             cleanUp(OHLC(symbolData))
...
returnSD <- function(symbol) {
  ...
  # How many standard deviations is the last weeks return and ATR
  #  compared to past 40 weeks?
  ...
  symD <- getYH_API(symbol = symbol, range = "1y", interval = "1d")

  symW <- symD[wday(symD$Date) == 6, ]
  # symW <- toWeekly(symD[,c("Open", "High", "Low", "Close", "Volume")],
  #                 keepLast=FALSE)
  symW <- symW[complete.cases(symW), ]
  ret <- (symW$Close - lag(symW$Close)) / lag(symW$Close)
  # ret <- dailyReturn(symW[,c("Open", "High", "Low", "Close")])
  atr <- ATR(symW[, c("High", "Low", "Close")], n = 40)[, "atr"]
  ...
  r <- data.frame(Symbol = symbol,
                  Return = last(zRtn),
                  AT = last(zATR), stringsAsFactors = FALSE)
  print(r)
  return(r)
}
...
stopLossTriggered <- function(Tickers) {
  ...
  # Extract each element of the list returned and turn into a dataframe
  sym <- sapply(t, function(l) as.character(l[1]))
...
getYH_API <- function(symbol = "ADE.OL", range = "1y", interval = "1wk") {
  # Get Yahoo Data from a REST API call
  # Possible ranges : 1d, 5d, 1mo, 3mo, 6mo, 1y, 5y, 10y, ytd, max
  # Possible intervals : 1m, 5m, 15m, 1d, 1wk, 1mo

  # Build the url
  endPoint <- paste0("https://query1.finance.yahoo.com/v8/finance/chart/",
                      symbol,
                     "?range=", range, "&interval=", interval,
                     "&lang=en&events=div%2Csplit")

  yh <- GET(endPoint)  # Get the REST date using httr library

  # Extract hex into readable JSON content
  yhContent <- content(yh, type = "text", encoding = "UTF-8")

  # use jsonlite library to extract the data from JSON into list
  yhFlatten <- fromJSON(yhContent, flatten = TRUE)

  # Get the indicator list which contains OHLCV data
  yhIndic <- yhFlatten$chart$result$indicators.quote[[1]]

  # Get the datetime list which is in UNIX format
  yhDate <- unlist(yhFlatten$chart$result$timestamp, use.names = FALSE)
  yhAdjClose <- unlist(yhFlatten$chart$result$indicators.adjclose,
                       use.names = FALSE)

  yhFinal <- data.frame(yhDate, yhIndic$open, yhIndic$high,
                        yhIndic$low, yhIndic$close,
                        yhAdjClose, yhIndic$volume,
                        stringsAsFactors = FALSE)
  colnames(yhFinal) <- c("Date", "Open", "High", "Low", "Close",
                         "AdjClose", "Volume")
  yhFinal$Date <- as_datetime(yhFinal$Date)
  return(yhFinal)
}
...
isNewHighLow <- function(ticker, lookBack = 40, ed = Sys.Date()) {
  ...
  # Enough daily data to convert into weeks
  startDate <- endDate - (lookBack * 10)

  # Get the data from Yahoo
  #  note: using 1wk interval does not seem to return correct data
  curStockData <- tryCatch(getYH_API(symbol = ticker,
                           range = "1y",
                           interval = "1d"),
                           error = function(e) {"error"}
                          )

  # If an error returned then exit
  if (class(curStockData) != "data.frame") {
     print("skipped")
     return(c(ticker, "skip", "skip", "skip", "skip"))
     }

  # Turn into weekly data, only want Fridays.
  # Need to deal with times Friday is a holiday
  curStockData <- curStockData[wday(curStockData$Date) == 6, ]
  ...
  #  names(curStockData) <- c("Open", "High", "Low", "Unadjust Close",
  #  "Volume", "Close")

  # Remove NA rows
  curStockData <- curStockData[!is.na(curStockData$Close), ]
  ...
  curStockData <- curStockData[index(curStockData) ==
                               unique(unique(index(curStockData))), ]
  ...
  curStockData[, c("Open", "High", "Low", "Close")] <-
              cleanUp(series = curStockData[, c("Open", "High",
                                                "Low", "Close")])
  ...
  if (curDateRow$Close >= curMax) {
    ...
    result <- c(ticker, "High", curDateRow$Close,
                ...
                mySNR(curStockData$Close, lookBack))
  ...
  # Pause for a random time between 0.5 and 1 sec to not overload
  #  the Yahoo API
...
breakOuts <- function(fileName, lookBack = 40, endDate) {
  ...
  df <- data.frame(matrix(unlist(r), nrow = length(r), byrow = TRUE),
                   stringsAsFactors = FALSE)
  ...
  i <- which(df$Type == "High" | df$Type == "Low" |
             df$Type == "skip" | df$Type == "data")
  df <- df[i, ]
...
weeklyRun <- function(risk = 500, endDate = Sys.Date()) {
  ...
  final$Stake <- floor(risk * as.numeric(final$CcyAdj) *
                       as.numeric(final$TickerMultiplier) / final$Stop)
